...
function calculateTotal() { let t=0; const wh=document.getElementById('sale_type').value==='wholesale'; const rt=document.getElementById('sale_type').value==='retail'; document.querySelectorAll('.item-row').forEach(r=>{const p=parseFloat(r.querySelector('.product-select').selectedOptions[0]?.dataset?.price||0);const q=parseInt(r.querySelector('.qty-input').value||0);let up=p;if(wh){const ci=r.querySelector('.wholesale-price-input input');if(ci&&ci.value)up=parseFloat(ci.value)||p;}else if(rt){const di=r.querySelector('.discount-price-input input');if(di&&di.value)up=parseFloat(di.value)||p;}const lt=up*q;r.querySelector('.line-total').textContent='TZS '+lt.toLocaleString();t+=lt;}); t+=calculateBottleTotal(); document.getElementById('grand-total').textContent='TZS '+t.toLocaleString(); validatePayments(); }

// Payment
function togglePaymentMode(m) { document.querySelectorAll('.payment-mode-option').forEach(e=>e.classList.remove('border-emerald-500','bg-emerald-50')); document.querySelector(`.payment-mode-option[data-mode="${m}"]`).classList.add('border-emerald-500','bg-emerald-50'); document.getElementById('singlePayment').classList.toggle('hidden',m!=='single'); document.getElementById('multiPayment').classList.toggle('hidden',m!=='multi'); if(m==='multi')validatePayments(); else{document.getElementById('submitBtn').disabled=false;document.getElementById('submitBtn').classList.remove('opacity-50');} }
