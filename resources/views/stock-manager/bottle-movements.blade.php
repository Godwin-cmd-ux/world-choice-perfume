...
@section('title', 'Bottle Stock History')
@section('header', 'Bottle Stock History')
...
<div class="bg-white rounded-xl shadow overflow-hidden">
    <div class="p-4 border-b border-gray-200 bg-gray-50">
        <form method="GET" class="flex gap-4 items-end">
            <div class="flex-1">
                <label class="block text-xs font-medium text-gray-500 mb-1">Volume</label>
                <select name="volume" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    <option value="">All Volumes</option>
                    @foreach($volumes as $volume)
                        <option value="{{ $volume }}" {{ request('volume') == $volume ? 'selected' : '' }}>{{ $volume }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex-1">
                ...
                <select name="type" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    ...
                    <option value="broken" {{ request('type') == 'broken' ? 'selected' : '' }}>Broken</option>
                </select>
            </div>
