                ...
                <div>
                    ...
                    <select name="sex_category"
                        ...
                        @foreach(['male' => 'Male', 'female' => 'Female', 'unisex' => 'Unisex', 'accessories' => 'Accessories', 'gift sets' => 'Gift Sets'] as $value => $label)
                            <option value="{{ $value }}" {{ old('sex_category', $product->sex_category) === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="flex gap-2 flex-wrap">
                @foreach($product->images as $img)
                    <div class="relative">
                        ...
                        <form action="{{ route('stock-manager.products.remove-image', $img->id) }}" method="POST" class="absolute -top-1 -right-1">
                            @csrf @method('DELETE')
                            <button type="submit" style="background-color: #F89A1E;" class="w-5 h-5 text-white rounded-full text-xs hover:opacity-90" title="Remove this image">
