        ...
        <form method="POST" action="{{ route('stock-manager.products.store') }}" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="fundamental_ingredient"
                        ...
                        @foreach(['Floral', 'Fresh/Citrus', 'Wood', 'Amber/Spicy', 'Fruity', 'Oud', 'Gourmand', 'Aromatic'] as $ingredient)
                            <option value="{{ $ingredient }}" {{ old('fundamental_ingredient') === $ingredient ? 'selected' : '' }}>{{ $ingredient }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <p class="text-[11px] text-gray-400 mt-1">You can upload multiple images. Max 50MB each.</p>
                </div>
            </div>
