...
<div class="max-w-lg mx-auto">
    <div class="bg-white rounded-xl shadow p-6" id="receipt">
        ...
        <div class="border-t border-b py-3 mb-4 text-sm">
            ...
            <div class="flex justify-between"><span>Type:</span><span class="px-2 py-0.5 rounded-full text-xs @if(($sale->sale_type ?? '') === 'wholesale') bg-blue-100 text-blue-800 @else bg-cyan-100 text-cyan-800 @endif">{{ ucfirst($sale->sale_type ?? 'retail') }}</span></div>
            ...
            <div class="flex justify-between"><span>Seller:</span><span>{{ $sale->cashier->name }}</span></div>
            @if(!empty($sale->notes))
    ...
    <div class="mt-4 text-center">
        <a href="{{ route('seller.sales.index') }}" class="text-cyan-700 hover:underline"><i class="fas fa-arrow-left mr-1"></i> Back to Sales</a>
    </div>
</div>
@endsection
