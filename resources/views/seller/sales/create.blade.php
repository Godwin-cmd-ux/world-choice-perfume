            ...
            <div class="bg-white rounded-xl shadow p-4">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="setSaleType('retail')" id="tab-retail"
                        class="flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all bg-cyan-600 text-white shadow">
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <div class="relative mb-3">
                    <input type="text" id="customerSearch" placeholder="Search by name or phone..."
                        class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500" autocomplete="off">
                ...
                <div id="newCustomerForm" class="hidden space-y-2 mb-3">
                    <input type="text" name="customer_name" id="newCustomerName" placeholder="New customer name" autocomplete="off" oninput="document.getElementById('customer_id').value='';" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500">
                    <input type="text" name="customer_phone" id="newCustomerPhone" placeholder="Phone (optional)" autocomplete="off" oninput="document.getElementById('customer_id').value='';" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500">
                </div>

                <button type="button" onclick="toggleNewCustomer()" id="newCustomerBtn" class="w-full text-sm text-cyan-700 hover:underline">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <input type="text" id="productSearch" placeholder="Search products..." class="w-48 px-3 py-1.5 border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500">
                </div>
                    ...
                    <div class="item-row flex gap-2 items-start">
                        <div class="flex-1 space-y-1">
                            ...
                            <input type="number" name="items[0][custom_price]" step="0.01" min="0" placeholder="Custom price"
                                class="w-full px-3 py-1.5 border rounded-lg text-xs focus:ring-2 focus:ring-cyan-500 wholesale-price-input hidden"
                ...
                <button type="button" onclick="addRow()" class="mt-3 text-cyan-700 hover:underline text-sm">
                    <i class="fas fa-plus mr-1"></i> Add Another Product
                </button>

                <div class="mt-4 pt-4 border-t flex justify-between items-center">
                    <span class="text-lg font-semibold">Total:</span>
                    <span id="grand-total" class="text-2xl font-bold text-cyan-700">TZS 0</span>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <button type="button" onclick="addBottleRow()" class="mt-3 text-cyan-700 hover:underline text-sm">
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <div class="flex gap-3 mb-4">
                    <label class="flex items-center gap-2 px-4 py-2 border rounded-lg cursor-pointer hover:bg-gray-50 transition payment-mode-option border-cyan-500 bg-cyan-50" data-mode="single">
                ...
                <div id="singlePayment" class="space-y-3">
                    <select name="payments[0][method]" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500">
                ...
                <div id="multiPayment" class="hidden space-y-3">
                    ...
                    <button type="button" onclick="addPaymentRow()" class="text-cyan-700 hover:underline text-sm"><i class="fas fa-plus mr-1"></i> Add Payment</button>
...
function setSaleType(type) {
    ...
    document.getElementById('tab-retail').className = type === 'retail'
        ? 'flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all bg-cyan-600 text-white shadow'
...
customerSearch.addEventListener('input', function() {
    ...
    searchTimeout = setTimeout(() => {
        fetch(`{{ route('api.customers.search') }}?q=${encodeURIComponent(q)}`)
            ...
            .then(data => {
                if (data.length === 0) {
                    customerResults.innerHTML = '<div class="px-3 py-2 text-sm text-gray-500">No customers found. <button type="button" onclick="createCustomerFromSearch()" class="text-cyan-700 hover:underline">Create new</button></div>';
                } else {
                    customerResults.innerHTML = data.map(c =>
                        `<div class="px-3 py-2 text-sm hover:bg-cyan-50 cursor-pointer border-b last:border-0" onclick="selectCustomer(${c.id}, '${(c.name||'').replace(/'/g,"\\'")}', '${(c.phone||'').replace(/'/g,"\\'")}')">
...
function togglePaymentMode(mode) {
    document.querySelectorAll('.payment-mode-option').forEach(el => el.classList.remove('border-cyan-500', 'bg-cyan-50'));
    document.querySelector(`.payment-mode-option[data-mode="${mode}"]`).classList.add('border-cyan-500', 'bg-cyan-50');
...
function addBottleRow() {
    ...
    const newRow = firstRow.cloneNode(true);
    newRow.querySelectorAll('select, input').forEach(el => {
        if (el.name) el.name = el.name.replace(/\d+/, bottleRowIndex);
    });
    newRow.querySelector('.bottle-qty-input').value = 1;
    newRow.querySelector('.bottle-price-input').value = '';
    newRow.querySelector('.bottle-line-total').textContent = 'TZS 0';
...
function bindBottleEvents() {
    document.querySelectorAll('.bottle-qty-input').forEach(input => {
        input.oninput = function() { calculateTotal(); };
    });
    document.querySelectorAll('.bottle-price-input').forEach(input => {
        input.oninput = function() { calculateTotal(); };
    });
}
...
function calculateBottleTotal() {
    ...
    document.querySelectorAll('.bottle-item-row').forEach(row => {
        ...
        row.querySelector('.bottle-line-total').textContent = 'TZS ' + lineTotal.toLocaleString();
        total += lineTotal;
    });
    return total;
}
