...
<div class="mb-4 flex flex-wrap items-center justify-between gap-3">
    <form method="GET" class="bg-white rounded-xl shadow px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        <button type="submit" class="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-lg text-sm"><i class="fas fa-filter mr-1"></i>Filter</button>
        @if(request('status'))
            <a href="{{ route('seller.orders.index') }}" class="text-sm text-gray-500 hover:text-gray-700"><i class="fas fa-times mr-1"></i>Clear</a>
        @endif
    </form>
</div>
            ...
            <tbody>
                @forelse($orders as $order)
                    <tr class="border-t hover:bg-gray-50">
                        ...
                        <td class="px-4 text-center">
                            <a href="{{ route('seller.orders.show', $order->id) }}" class="text-cyan-600 hover:underline mr-2"><i class="fas fa-eye"></i></a>
                            @php
                            ...
                            @else
                                ...
                                @if($next)
                                    <form action="{{ route('seller.orders.update-status', $order->id) }}" method="POST" class="inline">
                                        @csrf
