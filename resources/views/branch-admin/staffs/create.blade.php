...
@section('subtitle', 'Register a staff member under your branch')
@section('header-actions')
    <a href="{{ route('branch-admin.staffs.index') }}" class="text-sm text-gray-500 hover:text-gray-700">
        <i class="fas fa-arrow-left mr-1"></i> Back to Staffs
    </a>
@endsection
@section('content')
<form method="POST" action="{{ route('branch-admin.staffs.store') }}" class="max-w-2xl bg-white rounded-xl border border-gray-200 p-6">
    @csrf
    <div class="space-y-5">
        <div class="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-800">
            <i class="fas fa-store mr-1"></i> Branch: <span class="font-semibold">{{ $branchName ?? '#' . auth()->user()->branch_id }}</span>
        </div>
    ...
    <div class="flex gap-3 mt-6 pt-5 border-t border-gray-100">
        <a href="{{ route('branch-admin.staffs.index') }}" class="px-5 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Cancel</a>
