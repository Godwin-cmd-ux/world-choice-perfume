...
function computeSaleTotal() { let t=0; const wh=document.getElementById('sale_type').value==='wholesale'; document.querySelectorAll('.item-row').forEach(r=>{const p=parseFloat(r.querySelector('.product-select').selectedOptions[0]?.dataset?.price||0);const q=parseInt(r.querySelector('.qty-input').value||0);const ci=r.querySelector('.wholesale-price-input');let up=p;if(wh&&ci&&ci.value)up=parseFloat(ci.value)||p;const lt=up*q;r.querySelector('.line-total').textContent='TZS '+lt.toLocaleString();t+=lt;}); return t+calculateBottleTotal(); }
...
// Payment
function togglePaymentMode(m) { document.querySelectorAll('.payment-mode-option').forEach(e=>e.classList.remove('border-amber-500','bg-amber-50')); document.querySelector(`.payment-mode-option[data-mode="${m}"]`).classList.add('border-amber-500','bg-amber-50'); const single=document.getElementById('singlePayment'),multi=document.getElementById('multiPayment'); single.classList.toggle('hidden',m!=='single'); multi.classList.toggle('hidden',m!=='multi'); single.querySelectorAll('select,input').forEach(e=>e.disabled=m!=='single'); multi.querySelectorAll('select,input').forEach(e=>e.disabled=m!=='multi'); if(m==='multi')validatePayments(); else{document.getElementById('submitBtn').disabled=false;document.getElementById('submitBtn').classList.remove('opacity-50');} }
