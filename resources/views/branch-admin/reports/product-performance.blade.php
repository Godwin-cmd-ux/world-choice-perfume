...
@section('header', 'Product Performance: ' . $startDate->format('M d') . ' - ' . $endDate->format('M d, Y'))
            ...
            <tbody>
                @forelse($report as $index => $item)
                    <tr class="border-t hover:bg-gray-50">
                        ...
                        <td class="px-4 text-right">{{ $item['total_sold'] }}</td>
                        <td class="px-4 text-right font-bold text-green-700">TZS {{ number_format($item['total_revenue']) }}</td>
                    </tr>
                @empty
